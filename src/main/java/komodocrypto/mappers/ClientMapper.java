...
public interface ClientMapper {
    ...
    @Insert("INSERT INTO `komodoDB`.`oauth_client_details` " +
            "(`client_id`, `client_secret`, `scope`, `authorized_grant_types`) " +
            "VALUES (#{client_id}, #{client_secret}, #{scope}, #{authorized_grant_types}); ")
    public boolean insertClient(OAuth2Client client);


    @Select("SELECT * FROM `komodoDB`.`oauth_client_details` WHERE `client_id` = #{client_id} ")
    public OAuth2Client findClient(String client_id);


    @Update("UPDATE `komodoDB`.`oauth_client_details`" +
    ...
    @Delete("DELETE FROM `komodoDB`.`oauth_client_details`" +
            "WHERE client_id=#{client_id}")
